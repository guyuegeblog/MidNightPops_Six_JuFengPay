...
import com.third.app.Model.Live_Id;
import com.third.app.Model.ProgramInfo;
import com.third.app.Model.ProgramList;
...
import com.third.app.View.SelfGridView;
...
public class Live_MultiAdapter extends BaseMultiItemQuickAdapter<Multi, BaseViewHolder> {
    ...
    protected void convert(final BaseViewHolder baseViewHolder, final Multi multi) {
        switch (baseViewHolder.getItemViewType()) {
            case Multi.LIVE_ITEM:
                ...
                Glide.with(mContext).
                        load(multi.getLiveInfo().getClientPic_Url())
                ...
                Glide.with(mContext).
                        load(multi.getLiveInfo().getLogo_url())
                        .placeholder(R.drawable.allloading).
                        error(R.drawable.allloading).into(live_logo);

                if (multi.getLiveInfo().getIsLook().equals("1")) {
                ...
                ((TextView) baseViewHolder.getView(R.id.live_name)).setText("《" + multi.getLiveInfo().getTv_name() + "》");
    ...
    private void doPlay_Do(Multi multi) {
        ...
        if (!VipTool.userIsLoginSucces(mContext)) {
            T.showTextCenterToast(mContext, "请您到会员菜单进行登录");
            return;
        } else {
    ...
    private void doData(BaseViewHolder baseViewHolder, int position, Multi multi) {
        ...
        infoLive.setLive_id(Integer.parseInt(multi.getLiveInfo().getId()));
    ...
    private void do_view(String jsonData) {
        ...
        ((ProgressBar) baseViewHolders.getView(R.id.more_progress)).setVisibility(View.GONE);
        String json = AesUtils.getInstance().decrypt(jsonData);
            ...
            btn_pay.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (choice.equals("zfb")) {
                        mContext.startActivity(new Intent(mContext, WXPayEntryActivity.class).putExtra("diaologPay", "zfb"));
                    } else {
                        mContext.startActivity(new Intent(mContext, WXPayEntryActivity.class).putExtra("diaologPay", "wx"));
                    }
                }
            });
